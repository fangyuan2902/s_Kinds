...
class Hud: UIView {
    ...
    class func showHudView(message:String,autoHiden:Bool){
        let hud = Hud.init(frame: mainBounds())
        ...
        let size = nsText.boundingRect(with: CGSize.init(width: adaption(width: 200), height: mainHeight() * 0.5), options: [.usesLineFragmentOrigin,.usesFontLeading], attributes: [NSAttributedStringKey.font : UIFont.systemFont(ofSize: adaption(width: 16))], context: nil).size
